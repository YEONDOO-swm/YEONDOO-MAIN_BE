...
public class HistoryController {
    ...
    @GetMapping("/search")
    public ResponseEntity historySearch(@RequestParam String username){
    ...
    public ResponseEntity historySearchResult(@RequestParam String username, @PathVariable Long resultId){
        if(UserValidator.isNotValidName(username)){return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);}
    ...
    @GetMapping("/trash")
    public ResponseEntity historySearchTrash(@RequestParam String username){
        if(UserValidator.isNotValidName(username)){return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);}
        return new ResponseEntity<>(historyService.historySearchTrash(username), HttpStatus.OK);

    }
    @PostMapping("/trash")
    public ResponseEntity historySearchTrash(@RequestParam String username, @RequestBody List<String> papers){
        if(UserValidator.isNotValidName(username)){return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);}
    ...
    public ResponseEntity historyPaper(@RequestParam String username){
        if(UserValidator.isNotValidName(username)){return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);}
